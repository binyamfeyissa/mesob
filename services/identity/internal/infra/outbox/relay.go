...
// If KafkaBrokers is empty or unreachable, events are still marked published so
// the table never grows unboundedly.
type Relay struct {
	DB           *pgxpool.Pool
	KafkaBrokers string
}
	...
	for {
		...
		case <-ticker.C:
			if err := r.flush(ctx); err != nil {
				log.Error().Err(err).Msg("identity outbox flush error")
			}
		}
	}
}
...
func (r *Relay) flush(ctx context.Context) error {
	...
	if r.KafkaBrokers != "" {
		...
		if err := writer.WriteMessages(ctx, msgs...); err != nil {
			...
			log.Warn().Err(err).Msg("identity outbox: kafka unreachable, skipping forward")
		}
		writer.Close()
	}
	...
	log.Info().Int64("count", tag.RowsAffected()).Msg("identity outbox flushed")
	return nil
}
