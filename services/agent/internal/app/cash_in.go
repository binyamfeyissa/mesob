package app

import (
	"context"

	"github.com/gofrs/uuid"
	kiterr "github.com/mesob-wallet/go-kit/errors"
)

type CashInInput struct {
	AgentUserID    uuid.UUID
	UserMSISDN     string
	AmountMinor    int64
	CapturedAt     string
	IdempotencyKey string
}

type CashInOutput struct {
	TransactionID     string `json:"transaction_id"`
	AgentFloatMinor   int64  `json:"agent_float_minor"`
	UserCreditedMinor int64  `json:"user_credited_minor"`
}

type CashInUseCase struct {
	Agents   AgentRepository
	Ledger   LedgerClient
	Identity IdentityClient
	Events   EventPublisher
}

func (uc *CashInUseCase) Execute(ctx context.Context, in CashInInput) (*CashInOutput, error) {
	// 1. Find agent by user_id, verify ACTIVE
	agent, err := uc.Agents.FindByUserID(ctx, in.AgentUserID)
	if err != nil {
		return nil, &kiterr.DomainError{Code: "AGENT_NOT_FOUND", Message: "agent not found"}
	}
	if !agent.IsActive() {
		return nil, &kiterr.DomainError{Code: "AGENT_SUSPENDED", Message: "agent not active"}
	}
	if in.AmountMinor <= 0 {
		return nil, &kiterr.DomainError{Code: "INVALID_AMOUNT", Message: "amount must be positive"}
	}

	// 2. Find user by MSISDN via IdentityClient
	if uc.Identity == nil {
		return nil, &kiterr.DomainError{Code: "IDENTITY_UNAVAILABLE", Message: "identity service not configured"}
	}
	_, userAccountID, err := uc.Identity.FindUserByMSISDN(ctx, in.UserMSISDN)
	if err != nil {
		return nil, &kiterr.DomainError{Code: "USER_NOT_FOUND", Message: "user not found"}
	}

	// 3. Post ledger: agent_float_account(D) → user_wallet(C)
	floatAccountID := ""
	if agent.FloatAccountID != nil {
		floatAccountID = agent.FloatAccountID.String()
	}
	txnID, err := uc.Ledger.PostTransaction(ctx, "CASH_IN", in.IdempotencyKey, in.AgentUserID.String(), "AGENT", []LedgerEntry{
		{AccountID: floatAccountID, Direction: "D", AmountMinor: in.AmountMinor},
		{AccountID: userAccountID, Direction: "C", AmountMinor: in.AmountMinor},
	})
	if err != nil {
		return nil, err
	}

	// 4. Get updated float balance
	var floatMinor int64
	if uc.Ledger != nil && floatAccountID != "" {
		floatMinor, _ = uc.Ledger.GetBalance(ctx, floatAccountID)
	}

	// 5. Publish events
	if uc.Events != nil {
		if agent.FloatIsLow(floatMinor) {
			uc.Events.Publish(ctx, "AgentFloatLow", agent.ID.String(), map[string]any{
				"float_minor": floatMinor,
			})
		}
		uc.Events.Publish(ctx, "CashInRecorded", txnID, map[string]any{
			"agent_id":     agent.ID.String(),
			"user_msisdn":  in.UserMSISDN,
			"amount_minor": in.AmountMinor,
		})
	}

	return &CashInOutput{
		TransactionID:     txnID,
		AgentFloatMinor:   floatMinor,
		UserCreditedMinor: in.AmountMinor,
	}, nil
}
