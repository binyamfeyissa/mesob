	...
	for {
		...
		case <-ticker.C:
			if err := r.flush(ctx); err != nil {
				log.Error().Err(err).Msg("iqub outbox flush error")
			}
		}
	}
}
...
func (r *Relay) flush(ctx context.Context) error {
	...
	if r.KafkaBrokers != "" {
		...
		if err := writer.WriteMessages(ctx, msgs...); err != nil {
			...
			log.Warn().Err(err).Msg("iqub outbox: kafka unreachable, skipping forward")
		}
		writer.Close()
	}
	...
	log.Info().Int64("count", tag.RowsAffected()).Msg("iqub outbox flushed")
	return nil
}
